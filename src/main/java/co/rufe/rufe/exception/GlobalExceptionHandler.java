...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    /**
     * Maneja excepciones de argumentos inválidos (ej. reglas de negocio) (HTTP 400 Bad Request).
    ...
    /**
     * Maneja excepciones relacionadas con problemas de base de datos de Spring (DataAccessException).
    ...
    public ResponseEntity<ErrorResponse> handleDataAccessException(DataAccessException ex, WebRequest request) {
        ...
        // Puedes añadir más lógica para otros tipos de DataAccessException si es necesario.

        ErrorResponse errorResponse = new ErrorResponse(
                LocalDateTime.now(),
                HttpStatus.INTERNAL_SERVER_ERROR.value(), // Podría ser 409 para DuplicateKey, pero 500 para genéricos
                HttpStatus.INTERNAL_SERVER_ERROR.getReasonPhrase(),
                message,
                request.getDescription(false)
        );
        // Para DuplicateKeyException, se podría retornar HttpStatus.CONFLICT.
        // Pero dado que DataAccessException es muy genérica, un 500 es un buen fallback.
        return new ResponseEntity<>(errorResponse, HttpStatus.INTERNAL_SERVER_ERROR);
    }

    /**
     * Sobrescribe el método para manejar `MethodArgumentNotValidException` (errores de @Valid) (HTTP 400 Bad Request).
     * Mensaje diciente: "Los datos de entrada no son válidos."
     */
    @Override
    protected ResponseEntity<Object> handleMethodArgumentNotValid(
            MethodArgumentNotValidException ex, HttpHeaders headers, org.springframework.http.HttpStatusCode status, WebRequest request) {
        ...
        // Puedes optar por devolver todos los errores de campo en la respuesta si lo deseas,
        ...
        // Si quisieras incluir los detalles: errorResponse.setDetails(errors.toString()); (requiere un campo 'details' en ErrorResponse)
        return new ResponseEntity<>(errorResponse, HttpStatus.BAD_REQUEST);
    }

    /**
     * Manejador genérico para cualquier otra excepción no controlada (HTTP 500 Internal Server Error).
